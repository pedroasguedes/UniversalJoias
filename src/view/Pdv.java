...
public class Pdv extends JDialog {
	...
	public Pdv () {
		...
		imgBarCode.setIcon(new ImageIcon(Pdv .class.getResource("/img/barcode.png")));
		...
		btnAdicionar.setIcon(new ImageIcon(Pdv .class.getResource("/img/create.png")));
		...
		btnAlterar.setIcon(new ImageIcon(Pdv .class.getResource("/img/update.png")));
		...
		btnExcluir.setIcon(new ImageIcon(Pdv .class.getResource("/img/delete.png")));
		...
		txtProdEntrada.setBounds(520, 281, 142, 20);
		...
		txtProdValidade.setBounds(520, 331, 142, 20);
		getContentPane().add(txtProdValidade);
		
		lblNewLabel_1_2_1_4 = new JLabel("Venda");
		lblNewLabel_1_2_1_4.setBounds(454, 433, 60, 14);
		getContentPane().add(lblNewLabel_1_2_1_4);
		
		txtProdVenda = new JTextField();
		txtProdVenda.setBounds(520, 430, 89, 20);
	...
	private JLabel lblNewLabel_1_2_1_4;
	...
	private void setarCaixasTexto() {
			...
			try {
				...
				if (rs.next()) {	
					...
					//Formatar o valor do JCalendar para inserção correta no JText
					 ...
					 txtProdLocal.setText(rs.getString(12));
					 txtProdLucro.setText(rs.getString(13));
					 txtProdVenda.setText(rs.getString(14));
		...
		private void adicionarProduto() {
			...
			if (txtProdNome.getText().isEmpty()) {
				...
				// lógica principal
				String create = "insert into produtos(barcode,produto,descricao,fabricante, datacad, dataval, estoque,estoquemin,unidade,custo,localizacao,lucro,venda, idfor) values(?,?,?, ?,?,?,?,?,?,?,?,?,?,?)";
				try {
					// estabelecer conexão
					Connection con = dao.conectar();
					// Preparar a execução da query
					...
					SimpleDateFormat formatadorEntrada = new SimpleDateFormat("yyyyMMdd");
					String dataCad = formatadorEntrada.format(txtProdEntrada.getDate());
					pst.setString(5, dataCad);

					SimpleDateFormat formatadorValidade = new SimpleDateFormat("yyyyMMdd");
					String dataVal = formatadorValidade.format(txtProdValidade.getDate());
					...
					pst.setString(13,txtProdVenda.getText());
		...
		private void alterarFornecedor() {
			...
			if (txtProdNome.getText().isEmpty()) {
				...
				String update = "update produtos set barcode=?,produto=?,descricao=?,fabricante=?,estoque=?,estoquemin=?,unidade=?, custo=?, localizacao=?,lucro=?,venda=? where codigo =?";
				try {
					Connection con = dao.conectar();
					...
					pst.setString(11,txtProdVenda.getText());
					//pst.setString(12, txtForID.getText());
					pst.setString(12,txtProdID.getText());
					pst.executeUpdate();
		...
		private void limparCampos() {
			...
			btnAlterar.setEnabled(false);
			btnExcluir.setEnabled(false);
			txtProdEntrada.setDate(null);
			txtProdValidade.setDate(null);
		}
}//fim